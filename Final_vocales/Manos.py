...
nombre = 'letra_A' #podemos poner mano derecha y correrlo  #A E I O U
direccion = __path__=r'C:\Users\karim\Desktop\IAPeteVScode\Dedos\fotos\e' #validación despues
    ...
    if resultado.multi_hand_landmarks: #Si hay algo en los resultados entramos al if
        for mano in resultado.multi_hand_landmarks:  #Buscamos la mano dentro de la lista de manos que nos da el descriptor
            ...
            if len(posiciones) != 0:
                ...
                x1,y1 = (pto_i5[1]-100),(pto_i5[2]-100) #Obtenemos el punto incial y las longitudes  80
                ancho, alto = (x1+200),(y1+200) #80
                x2,y2 = x1 + ancho, y1 + alto
                dedos_reg = copia[y1:y2, x1:x2]
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 3)
            dedos_reg = cv2.resize(dedos_reg,(200,200), interpolation = cv2.INTER_CUBIC) #Redimensionamos las fotos
            cv2.imwrite(carpeta + "/Dedos_{}.jpg".format(cont),dedos_reg) #Mano_  o tambien como "A"
            cont = cont + 1
